...
export default async function HomePage() {
  ...
  // Keep morph names unique on the page — trending must not repeat a slug shown above.
  const shownSlugs = new Set([feature?.slug, ...supporting.map((t) => t.slug)]);
  const trending = [...types]
    .sort((a, b) => b.reviews_count - a.reviews_count)
    .filter((t) => !shownSlugs.has(t.slug))
    .slice(0, 8);
  ...
  return (
    <div className="rz">
      ...
      <div className="relative -mt-[68px]">
        <section className="sticky top-0 flex h-screen flex-col items-center justify-center px-4 pb-12 pt-20 text-center sm:px-6">
          ...
          <HeroReveal className="relative w-full max-w-3xl">
            ...
            <h1
              data-hero-heading
              className="rz-serif mb-4 text-[clamp(2.2rem,7vw,4.75rem)] font-semibold leading-[1.02] tracking-[-0.02em] text-white"
            >
              Book the moments<br />that matter.
            </h1>
            <p
              data-hero-sub
        ...
        <div className="relative z-10 overflow-hidden rounded-t-[calc(var(--r)+22px)] bg-paper shadow-[0_-40px_80px_-30px_rgba(15,10,5,0.5)] ring-1 ring-white/30">
          <LineArt variant="waves" opacity={0.75} />
      ...
      <div className="relative z-10 bg-paper">
        {/* how Réserve works — full-width storytelling, first in the flow */}
          ...
          <>
            ...
            <section className="py-11">
              <div className="mx-auto max-w-6xl px-4 sm:px-6">
                ...
                <div className="relative overflow-hidden rounded-[calc(var(--r)+10px)] border border-[var(--panel-border)] bg-surface p-3 shadow-[0_40px_90px_-50px_rgba(60,45,30,0.5)] sm:p-4">
                  <LineArt opacity={0.6} />
                  <div className="relative grid gap-3 sm:gap-4 lg:grid-cols-2">
                    {feature && (
                      <ExperienceCard
                        type={feature}
                        wide
                        vtName={`venue-${feature.slug}`}
                        className="h-[420px] lg:h-[556px]"
                      />
                    )}
                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                      {supporting.map((type) => (
                        <ExperienceCard key={type.id} type={type} vtName={`venue-${type.slug}`} className="h-[200px] lg:h-[270px]" />
                      ))}
                    </div>
                  </div>
                </div>
            ...
            {/* one curated collection — the fast-movers, full-width dark band */}
            <CollectionRow
              dark
              vt
              eyebrow="Moving fast"
